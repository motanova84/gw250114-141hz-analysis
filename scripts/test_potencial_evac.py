...
def test_E_vac_function():
    ...
    # Test 3: Verificar que el valor en el mínimo es menor que en valores cercanos
    try:
        E_min = Evac(R_min)
        E_before = Evac(R_min * 0.9)
        E_after = Evac(R_min * 1.1)
        assert E_min <= E_before, "El mínimo debe ser menor que el valor anterior"
        assert E_min <= E_after, "El mínimo debe ser menor que el valor posterior"
    ...
    try:
        ...
        term2 = beta * zeta_prime * R_test**(-2)
        term3 = gamma * (Lambda**2) * (R_test * lP)**2
        term4 = delta * np.sin(np.log(R_test) / np.log(b))**2
